...
call s:DefineVariable ("g:errormarker_erroricon", has('win32') ? expand ("~/vimfiles/icons/error.bmp") : "/usr/share/icons/gnome/16x16/status/dialog-error.png")

" Defines the icon to show for warnings in the gui
call s:DefineVariable ("g:errormarker_warningicon", has('win32') ? expand ("~/vimfiles/icons/warning.bmp") : "/usr/share/icons/gnome/16x16/status/dialog-warning.png")

" Defines the text (one character) to show for errors in the gui
call s:DefineVariable ("g:errormarker_errortext", "E")

" Defines the text (one character) to show for warnings in the gui
call s:DefineVariable ("g:errormarker_warningtext", "W")
...
execute "sign define errormarker_error text=" . g:errormarker_errortext . " linehl=" . g:errormarker_errorgroup . s:erroricon

execute "sign define errormarker_warning text=" . g:errormarker_warningtext . " linehl=" . g:errormarker_warninggroup . s:warningicon
...
function! s:SetErrorMarkers()
    ...
    for l:d in getqflist()
        ...
        if strlen (l:d.type) && stridx (g:errormarker_warningtypes, l:d.type) >= 0
            let l:name = "errormarker_warning"
        else
            let l:name = "errormarker_error"
        endif
        execute ":sign place " . l:key . " line=" . l:d.lnum . " name=" . l:name . " buffer=" . l:d.bufnr
    endfor
endfunction
